...
def interpolate_to_common_grid(df, common_grid_shape):
    ...
    for char_len in char_lens:
        print("Interpolating: " + str(char_len))

        df_char = df[df['CharLen'] == char_len]
        
        # Swap the order of x and y coordinates when creating the points array
        points = df_char[['X', 'Y']].values
        values_height = df_char['Height'].values
        values_x_velocity = df_char['X_velocity'].values
        values_y_velocity = df_char['Y_velocity'].values
        ...
        df_interpolated = pd.DataFrame({'CharLen': np.full(height_grid.size, char_len),
                                        'X': x_grid.ravel(),
                                        'Y': y_grid.ravel(),
